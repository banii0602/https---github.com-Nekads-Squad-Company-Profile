    ...
    <div class="row">
      <div class="col-md-12">
        <form action="{{ route('artikels.update', $artikel->id) }}" method="POST" enctype="multipart/form-data">
          ...
          <div class="form-group">
            <label for="">Deskripsi</label>
            <textarea name="description2"  cols="30" rows="10" class="form-control" placeholder="Deskripsi II">{{ $artikel->description2 }}</textarea>
          </div>
          @error('description2')
          <small style="color:red">{{ $message }}</small>
          @enderror
          <div class="form-group">
            <label for="">Deskripsi</label>
            <textarea name="description3"  cols="30" rows="10" class="form-control" placeholder="Deskripsi III">{{ $artikel->description3 }}</textarea>
          </div>
          @error('description3')
          <small style="color:red">{{ $message }}</small>
          @enderror
